...
class Dataset(object):
    ...
    def iterate_forever(self, batch_size):
        while True:
            for bar in self.iterate_once(batch_size):
                yield bar
...
class PerceptronDataset(Dataset):
    def __init__(self, model):
        ...
        super(PerceptronDataset,self).__init__(x, np.expand_dims(y, axis=1))
    ...
    def iterate_once(self, batch_size):
        ...
        for i, (x, y) in enumerate(super(PerceptronDataset,self).iterate_once(batch_size)):
            yield x, y
...
class RegressionDataset(Dataset):
    def __init__(self, model):
        ...
        y = np.sin(x)
        super(RegressionDataset,self).__init__(x, y)
    ...
    def iterate_once(self, batch_size):
        for x, y in super(RegressionDataset,self).iterate_once(batch_size):
            yield x, y
...
class DigitClassificationDataset(Dataset):
    def __init__(self, model):
        ...
        super(DigitClassificationDataset,self).__init__(train_images, train_labels_one_hot)
    ...
    def iterate_once(self, batch_size):
        ...
        for i, (x, y) in enumerate(super(DigitClassificationDataset,self).iterate_once(batch_size)):
            yield x, y
